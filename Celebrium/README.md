...
NOTE: The user can only refer an even number of 250s that are on network 1. Therefore nn and denomination parameters are not required.

The refer service allows a Root RAIDA to know the CloudCoins that are being used as referrals. The user can refer all the coins they want without paying. 
...
Once every day, a Chron job will call the Count service on corresponding RAIDA in subdomains. The Chron job will then count
the number of CloudCoins dedicated to that nn in the Ans table. It will need to use the domains table to know the conversion rate. 
...
This allows the Client to coordinate with the RAIDA so that new coins created will be of the same serial numbers.
...
6. Make it possible for the RAIDA to synchronize data without the users help. 

In order to achieve this. the user may store their data in three classes: 
Class 0 is striped. 
Class 01 is stripped, and then a mirror of that stripe is stored offsent by one RAIDA
Class 011 (or just class 2_) The data is stored with a third mirror offsent by two RAIDA. 
...
![Screenshot 5](https://github.com/CloudCoinConsortium/CloudCoin/blob/master/Celebrium/c1.pn
	"status":"success",
#  Root RAIDA and the Celebrium
